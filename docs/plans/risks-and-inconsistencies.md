...
### B7. `[RESOLVED]` `secret:provider` is referenced as a capability but absent from the capability examples
- **Status:** RESOLVED (2026-05-28) — superseded by ADR-0009.
- **Original gap:** `mote-registry` needed a contract entry for `secret:provider`. The prose said it was non-exclusive but effectively singular via the exclusive `password-manager:provider` capability.
- **Resolution:** The "effectively singular via exclusive password-manager" rationale was wrong. ADR-0009 (Accepted 2026-05-28) established that `password-manager:provider` is **non-exclusive** (multiple managers may coexist). Resolution is unambiguous not because only one provider exists, but because the **caller names the provider explicitly** (`secrets.lua` must supply `provider = "<plugin-name>"`; the runtime invokes exactly that fulfiller via `invoke_capability_on` — no fan-out). Both `secret:provider` (non-exclusive, fan-out) and `password-manager:provider` (non-exclusive, fan-out) are now in `capabilities/v1.toml` with correct contracts. DESIGN.md and the registry have been reconciled to this model.
